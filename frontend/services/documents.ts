...
export async function listDocuments(): Promise<DocumentListResponse> {
  const response = await fetch(`${API_BASE}/api/v1/documents`, { cache: "no-store" });
  if (!response.ok) throw new Error("Failed to load documents");
  return response.json();
}

export async function getDocument(documentId: string): Promise<DocumentItem> {
  const response = await fetch(`${API_BASE}/api/v1/documents/${documentId}`, { cache: "no-store" });
  if (!response.ok) throw new Error("Failed to load document");
  return response.json();
}

export async function getDocumentChunks(documentId: string): Promise<DocumentChunksResponse> {
  const response = await fetch(`${API_BASE}/api/v1/documents/${documentId}/chunks`, { cache: "no-store" });
...
export async function reindexDocument(documentId: string): Promise<{ ok: boolean; document_id: string; status: string }> {
  const response = await fetch(`${API_BASE}/api/v1/documents/${documentId}/reindex`, {
    method: "POST",
    cache: "no-store",
  });
