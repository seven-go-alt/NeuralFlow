...
export function Sidebar() {
  ...
  return (
    <aside className="hairline-panel hidden w-[292px] shrink-0 flex-col border-r bg-zinc-950/80 backdrop-blur md:flex animate-fade-in">
      <div className="flex h-16 items-center gap-2 border-b px-4">
        <div className="grid h-9 w-9 place-items-center rounded-lg border border-cyan-300/30 bg-cyan-300/15 text-sm font-bold text-cyan-100 font-mono">N</div>
        <div className="min-w-0">
          <div className="truncate text-sm font-semibold text-zinc-50">NeuralFlow</div>
          <div className="text-[11px] text-zinc-500 font-mono">Agent Runtime Platform</div>
        </div>
        <Badge tone="emerald" pulse className="ml-auto">
          live
        </Badge>
      </div>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2 text-center">
            {[
              ...
              <div key={label} className="rounded-md border border-white/10 bg-black/20 px-2 py-2">
                <div className="text-sm font-semibold text-zinc-100 font-mono">{value}</div>
              ...
              {modes.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-1 text-[11px] leading-4 text-zinc-500 font-mono">{item.description}</div>
                  </button>
                );
              })}
            </div>
